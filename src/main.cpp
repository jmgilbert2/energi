...
{
    double dDiff;
    CAmount nSubsidyBase = 4500000;

	int64_t period = nPrevHeight / consensusParams.nSubsidyHalvingInterval;

	if (period >= 8)
		nSubsidyBase = 1000000;
	else if (period == 7)
		nSubsidyBase = 1200000;
	else
		nSubsidyBase -= 500000 * period;

    // LogPrintf("height %u diff %4.2f reward %d\n", nPrevHeight, dDiff, nSubsidyBase);
    CAmount nSubsidy = nSubsidyBase * COIN / consensusParams.nSubsidyHalvingInterval; // consensus.nSubsidyHalvingInterval = 1 month

    // Hard fork to reduce the total block by the amount required for budget/superblocks
    CAmount nSuperblockPart = (nPrevHeight > consensusParams.nBudgetPaymentsStartBlock) ? 400000 * COIN / consensusParams.nSubsidyHalvingInterval : 0;

    return fSuperblockPartOnly ? nSuperblockPart : nSubsidy - nSuperblockPart;
}

CAmount GetMasternodePayment(int /*nHeight*/, const Consensus::Params& consensusParams)
{
	//##ENERGI+- Constant payment of 400000 coins per month
	CAmount ret = 400000 * COIN / consensusParams.nSubsidyHalvingInterval;
